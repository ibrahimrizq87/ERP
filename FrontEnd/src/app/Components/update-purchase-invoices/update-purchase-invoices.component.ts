...
@Component({
  selector: 'app-update-purchase-invoices',
  imports: [CommonModule,ReactiveFormsModule],
  templateUrl: './update-purchase-invoices.component.html',
  styleUrl: './update-purchase-invoices.component.css'
})
export class UpdatePurchaseInvoicesComponent implements OnInit{
  msgError: any[] = [];
      ...
      next: (response) => {
        if (response) {
          ...
          this.purchasesForm.patchValue({
            ...
            payementType:purchaseData.payment_type,
            account_id:purchaseData.account.id,
            supplier_id:purchaseData.supplier.id,
            amount : purchaseData.amount_letters,
  ...
  handleForm() {
    ...
    if (this.purchasesForm.valid) {
      ...
      const purchaseId = this.route.snapshot.paramMap.get('id');
      if (purchaseId){
      this._InvoiceService.updatePurchaseInvoice(purchaseId,formData).subscribe({
        next: (response) => {
          console.log(response);
          if (response) {
