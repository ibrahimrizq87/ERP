...
export class DocumentReportComponent {
...
loadReports(filters: { startDate?: string; endDate?: string; today?: boolean; thisYear?: boolean } = {}) {
  ...
  this._ReportsService.getPaymentDocumentReports(filters).subscribe({
    next: (response) => {
      if (response) {
        console.log('Product reports data:', response);
       
        this.box= response.box[0]; 
        this.bank= response.bank[0]; 
